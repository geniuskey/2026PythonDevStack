...
|----------|------|----------|-----------|
| **스케줄링** | APScheduler | 매일 아침 리포트 생성 | Yes |
| **CLI 도구** | click / typer | 명령줄 인터페이스 | Yes |
| **웹 스크래핑** | httpx + bs4 | 데이터 수집 | Yes |
| **브라우저 자동화** | playwright | 웹 테스트, 크롤링 | Yes |
| **파일 감시** | watchdog | 파일 변경 시 자동 처리 | Yes |
| **작업 큐** | dramatiq | 비동기 작업 처리 | Yes |
| **원격 실행** | fabric | 원격 서버 작업 | - |
| **워크플로우** | Dagster | 복잡한 자동화 체인 | Yes |
